		...
		case create
			...
			set instance_id (
				vultr instance create \
					...
					--region sgp \
					--tags vpn \
					--ssh-keys "d818f0ac-6fa3-4de0-b96f-9a5088f4449c,13cdbf4d-bc47-4a00-9b99-b08e8d1c39a5" \
			...
			while not test -n "$instance_ip"; or string match "0.*" "$instance_ip"
				set instance_ip  (vultr instance get $instance_id | grep -E "^MAIN IP" | awk '{print $3}')
				sleep 3
			end
			...
			scp -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" "root@$instance_ip:*.conf" .
		case delete
			vultr instance delete (
				vultr instance list | grep "\[vpn\]" | awk '{print $1}'
			)
	end
end
