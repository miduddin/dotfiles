...
Map("<Leader><Tab>d", "<Cmd>tabclose<CR>", "n", { desc = "Close tab" })
Map("<Leader><Tab>n", "<Cmd>tabnext<CR>", "n", { desc = "Next tab" })
Map("<Leader><Tab>p", "<Cmd>tabprevious<CR>", "n", { desc = "Previous tab" })
...
vim.api.nvim_create_autocmd("LspAttach", {
	callback = function(ev)
		...
		Map("K", function() vim.lsp.buf.hover({ max_width = 82 }) end, "n", { desc = "LSP Hover", buffer = ev.buf })
	end,
})
